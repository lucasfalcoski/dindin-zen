...
function UserMenu({ name, email, color, onSignOut }: { name: string; email: string; color: string; onSignOut: () => void }) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button onClick={() => setOpen(o => !o)} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 10px 4px 4px', borderRadius: 100, border: `1px solid ${C.topbarBorder}`, background: 'none', cursor: 'pointer', transition: 'background .15s' }}
      ...
      {open && (
        <div style={{ position: 'absolute', right: 0, top: '100%', marginTop: 6, background: '#fff', border: `1px solid ${C.topbarBorder}`, borderRadius: 12, minWidth: 200, boxShadow: '0 8px 24px rgba(0,0,0,.1)', zIndex: 100, overflow: 'hidden' }}>
          ...
          {[{ icon: User, label: 'Meu perfil', to: '/profile' }, { icon: Settings, label: 'Configurações', to: '/profile' }].map(item => (
    ...
    <>
      ...
      <aside className="din-sidebar" style={{ position: 'fixed', top: 0, left: 0, bottom: 0, width: 64, zIndex: 50, background: C.sidebarBg, display: 'flex', flexDirection: 'column', alignItems: 'center', paddingTop: 12, paddingBottom: 10, overflowY: 'auto', overflowX: 'visible' }}>
        ...
        <div style={{ width: '100%', borderTop: `1px solid ${C.sidebarSep}`, paddingTop: 3 }}>
          <Tip label="Sair">
            <button onClick={handleSignOut} style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '100%', height: 44, background: 'none', border: 'none', cursor: 'pointer', transition: 'background .15s' }}
      ...
      <div className="din-main" style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', background: C.pageBg }}>
        <header style={{ position: 'sticky', top: 0, zIndex: 40, background: C.topbarBg, borderBottom: `1px solid ${C.topbarBorder}`, height: 52, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <button onClick={() => setSearchOpen(true)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '5px 12px', borderRadius: 8, color: C.ink2, background: 'none', border: `1px solid ${C.topbarBorder}`, cursor: 'pointer', fontFamily: "'Cabinet Grotesk', sans-serif", fontSize: 12, fontWeight: 500 }}>
      ...
      <nav className="din-bottom-nav" style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: C.topbarBg, borderTop: `1px solid ${C.topbarBorder}`, height: 64, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'space-around' }}>
        {mobileSlots.map((slot, i) => !slot
          ...
          : <NavLink key={slot.to} to={slot.to} end={slot.to === '/'} style={({ isActive }) => ({ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2, padding: '4px 8px', textDecoration: 'none', flex: 1, color: isActive ? C.green : C.ink3, fontSize: 9, fontWeight: 600, fontFamily: "'Cabinet Grotesk', sans-serif", textTransform: 'uppercase', letterSpacing: '0.3px' })}>
